...
export default function HowItWorks({ steps }: Props) {
  ...
  return (
    <div className="relative mt-16 grid grid-cols-1 gap-10 lg:grid-cols-12 lg:gap-14">
      ...
      <div className="hidden lg:col-span-7 lg:block">
        <div className="sticky top-32">
          <div className="surface-card relative h-[460px] overflow-hidden">
            {/* radial highlight that follows the active step */}
            <div
              aria-hidden
              className="pointer-events-none absolute inset-0 transition-all duration-700"
              style={{
                background: `radial-gradient(35% 40% at ${20 + active * 30}% 30%, rgba(22,119,255,0.18), transparent 70%)`,
              }}
            />
...
function StepConfigure() {
  return (
    <motion.div {...fadeIn} className="absolute inset-0 flex items-center justify-center p-10">
      <div className="w-full max-w-md rounded-xl border border-white/10 bg-ink-900/80 p-5 shadow-card">
        <div className="mb-4 flex items-center justify-between">
          <span className="text-[11px] font-medium uppercase tracking-widest text-zinc-500">Custom Provider</span>
          <span className="rounded-full bg-accent-lime/10 px-2 py-0.5 font-mono text-[10px] text-accent-lime ring-1 ring-accent-lime/20">connected</span>
        </div>
        {[
          ...
          >
            <span className="w-20 flex-none font-mono text-[11px] text-zinc-500">{k}</span>
            <span className="truncate font-mono text-[12.5px] text-zinc-100">{v}</span>
          </motion.div>
        ))}
      </div>
    </motion.div>
    ...
    <motion.div {...fadeIn} className="absolute inset-0 flex items-center p-8">
      <div className="grid w-full grid-cols-2 gap-4">
        <div className="rounded-xl border border-white/10 bg-ink-900/80 p-4">
          <div className="mb-2 font-mono text-[10px] uppercase tracking-widest text-zinc-500">template</div>
          <pre className="overflow-hidden font-mono text-[11.5px] leading-6 text-zinc-300 whitespace-pre-wrap">{tpl.split('\n').map((ln, i) => (
            <motion.div key={i} initial={{ opacity: 0, x: -4 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 + i * 0.06 }}>
              <span className="mr-3 select-none text-zinc-700">{i + 1}</span>
              <span dangerouslySetInnerHTML={{
                __html: ln
                  .replaceAll(/(\{\%[^%]*\%\})/g, '<span style="color:#ff9ec7">$1</span>')
                  .replaceAll(/(\{\{[^}]*\}\})/g, '<span style="color:#8be7ff">$1</span>'),
              }} />
            </motion.div>
          ))}</pre>
        </div>
        <div className="rounded-xl border border-brand-500/20 bg-ink-900/80 p-4">
          <div className="mb-2 flex items-center gap-2 font-mono text-[10px] uppercase tracking-widest text-brand-300">
            <svg viewBox="0 0 24 24" className="h-3 w-3" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M5 12h14M13 6l6 6-6 6"/></svg>
            rendered
          </div>
          <pre className="font-mono text-[11.5px] leading-6 text-zinc-200 whitespace-pre-wrap">
            {rendered.split('').map((c, i) => (
              <motion.span key={i} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.6 + i * 0.008, duration: 0.2 }}>{c}</motion.span>
            ))}
          </pre>
        </div>
      </div>
    ...
    <motion.div {...fadeIn} className="absolute inset-0 flex items-center justify-center p-10">
      <div className="w-full max-w-lg rounded-xl border border-white/10 bg-ink-950/90 p-5 font-mono text-[12.5px]">
        <div className="mb-3 flex items-center gap-1.5">
          ...
          <span className="ml-3 text-[10px] text-zinc-500">openwarp · ready</span>
        </div>
        {[
          { t: '$ openwarp', delay: 0 },
          ...
          { t: 'Looking at frame 3, the panic happens', tone: 'soft', delay: 2.1 },
          { t: 'because the buffer is dropped before…', tone: 'soft', delay: 2.5 },
        ].map((ln, i) => (
          <motion.div
            key={i}
            ...
            className={
              ln.tone === 'lime' ? 'text-accent-lime' :
              ln.tone === 'cyan' ? 'text-accent-cyan' :
              ln.tone === 'soft' ? 'text-zinc-400 pl-3' :
              'text-zinc-100'
            }
          >
            {ln.t}
