...
settings-agent-providers-auth-login-description = Sign in with Codex to add a new Codex Auth provider. Tokens are stored in the system keychain per provider.
settings-agent-providers-auth-login-login = Log in with Codex
settings-agent-providers-auth-login-managed-secret = Managed by Codex Auth. Tokens are stored in the system keychain.
