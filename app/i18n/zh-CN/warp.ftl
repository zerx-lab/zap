...
settings-agent-providers-auth-login-description = 使用 Codex 登录并新增一个 Codex Auth 提供商。每个提供商的 token 会分别安全存储在系统密钥库。
settings-agent-providers-auth-login-login = 使用 Codex 登录
settings-agent-providers-auth-login-managed-secret = 由 Codex Auth 管理。Token 存储在系统密钥库。
