...
fn rewards() -> Vec<Reward> {
    vec![
        Reward {
            ...
            icon_width: 64.,
            icon_height: 64.,
            label: crate::t!("settings-referrals-reward-theme"),
        },
        Reward {
            ...
            icon_width: 56.,
            icon_height: 56.,
            label: crate::t!("settings-referrals-reward-keycaps"),
        },
        Reward {
            ...
            icon_width: 64.,
            icon_height: 64.,
            label: crate::t!("settings-referrals-reward-tshirt"),
        },
        Reward {
            ...
            icon_width: 64.,
            icon_height: 64.,
            label: crate::t!("settings-referrals-reward-notebook"),
        },
        Reward {
            ...
            icon_width: 64.,
            icon_height: 64.,
            label: crate::t!("settings-referrals-reward-cap"),
        },
        Reward {
            ...
            icon_width: 64.,
            icon_height: 64.,
            label: crate::t!("settings-referrals-reward-hoodie"),
        },
        Reward {
            ...
            icon_width: 48.,
            icon_height: 48.,
            label: crate::t!("settings-referrals-reward-hydroflask"),
        },
        Reward {
            ...
            icon_width: 50.,
            icon_height: 50.,
            label: crate::t!("settings-referrals-reward-backpack"),
        },
    ]
}
...
impl ReferralsPageView {
    pub fn new(referrals_client: Arc<dyn ReferralsClient>, ctx: &mut ViewContext<Self>) -> Self {
        ...
        // 启动期一次性 leak 成 'static —— 一个进程只会执行一次。
        let page_title: &'static str =
            Box::leak(crate::t!("settings-referrals-page-title").into_boxed_str());
        let page = PageType::new_monolith(ReferralsWidget::default(), Some(page_title), true);
        Self {
            page,
            referrals_client,
        ...
        match &self.api_state {
            ...
            ApiState::Ready { referral_info, .. } => {
                ...
                ctx.emit(ReferralsPageEvent::ShowToast {
                    message: crate::t!("settings-referrals-link-copied-toast"),
    ...
    fn handle_send_email_invite_response(
        ...
        match response {
            Ok(successful) => {
                ...
                ctx.emit(ReferralsPageEvent::ShowToast {
                    message: crate::t!("settings-referrals-email-success-toast"),
            ...
            Err(err) => {
                ...
                ctx.emit(ReferralsPageEvent::ShowToast {
                    message: crate::t!("settings-referrals-email-failure-toast"),
                    flavor: ToastFlavor::Error,
                });
            }
        }
...
impl ReferralsWidget {
    ...
    fn render_link_row(
        ...
        let (link_text, button_enabled) = match &view.api_state {
            ...
            ApiState::Loading => (crate::t!("settings-referrals-loading"), false),
            ApiState::Failed => (crate::t!("settings-referrals-link-error"), false),
        };
        ...
        Container::new(
            Flex::row()
                ...
                .with_child(self.render_button(
                    button_enabled,
                    &crate::t!("settings-referrals-copy-link-button"),
    ...
    fn render_email_row(
        ...
        let (button_text, button_enabled) = match &view.api_state {
            ApiState::Ready {
                email_state: SendEmailState::Idle,
                ..
            } => (crate::t!("settings-referrals-email-send-button"), true),
            ApiState::Ready {
                email_state: SendEmailState::Sending,
                ..
            } => (crate::t!("settings-referrals-email-sending-button"), false),
            _ => (crate::t!("settings-referrals-email-send-button"), false),
        };
    ...
    fn render_send_invite_section(
        ...
        Flex::column()
            .with_child(
                Container::new(
                    self.render_label(crate::t!("settings-referrals-link-label"), appearance),
                )
    ...
    fn render_signup_section(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        let sign_up_button = appearance
            ...
            .with_text_label(crate::t!("settings-referrals-sign-up"))
            .build()
        ...
        Flex::column()
            .with_child(
                Container::new(
                    appearance
                        .ui_builder()
                        .span(crate::t!("settings-referrals-anonymous-header"))
                        .with_style(UiComponentStyles {
    ...
    fn render_rewards_section(
        ...
        rewards_section.add_child(
            Container::new(
                appearance
                    .ui_builder()
                    .span(crate::t!("settings-referrals-reward-intro"))
                    .with_style(UiComponentStyles {
                    ...
                    FormattedTextElement::new(
                        FormattedText::new([FormattedTextLine::Line(vec![
                            ...
                            FormattedTextFragment::hyperlink(
                                crate::t!("settings-referrals-terms-link"),
                                TERMS_URL,
                            ),
    ...
    fn render_meter(&self, view: &ReferralsPageView, appearance: &Appearance) -> Box<dyn Element> {
        ...
        let rewards_list = rewards();
        for (index, reward) in rewards_list.iter().enumerate() {
            ...
            if index < (rewards_list.len() - 1) {
                ...
                let higher_threshold = rewards_list[index + 1].required_referral_count;
    ...
    fn render_claimed_referrals_count(
        ...
        let current_referrals_label = match claimed_count {
            1 => crate::t!("settings-referrals-claimed-count-singular"),
            _ => crate::t!("settings-referrals-claimed-count-plural"),
        };
