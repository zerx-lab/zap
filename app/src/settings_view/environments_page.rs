    ...
    fn format_timestamp_text(&self) -> String {
        let last_edited_part = self.last_edited_ts.map(|ts| {
            crate::t!(
                ...
                time = format_approx_duration_from_now_utc(ts.utc())
            )
        });
        let last_used_part = match self.last_used_ts {
            ...
            None => crate::t!("settings-environments-last-used-never"),
        };
                ...
                if let Some(result_client_id) = &result.client_id {
                    if *result_client_id == pending_client_id {
                        self.show_success_toast(
                            crate::t!("settings-environments-toast-create-success"),
                            ctx,
                        );
                    }
                }
            ...
            if let Some(pending_env_id) = self.pending_delete_env_id.take() {
                ...
                if let Some(server_id) = &result.server_id {
                    if server_id.uid() == pending_env_id.uid() {
                        self.show_success_toast(
                            crate::t!("settings-environments-toast-delete-success"),
                            ctx,
                        );
                    }
                }
        ...
        match event {
            UpdateEnvironmentFormEvent::Created {
                ...
                let Some(owner) = owner else {
                    self.show_error_toast(
                        crate::t!("settings-environments-toast-create-not-logged-in"),
                        ctx,
                    );
                    return;
                };
            ...
            UpdateEnvironmentFormEvent::Updated {
                ...
                let Some(existing_env) = CloudAmbientAgentEnvironment::get_by_id(env_id, ctx)
                    ...
                    self.show_error_toast(
                        crate::t!("settings-environments-toast-save-not-found"),
                        ctx,
                    );
                    return;
                };
        ...
        match action {
            ...
            EnvironmentsPageAction::ShareToTeam(env_id) => {
                let Some(team_uid) = UserWorkspaces::as_ref(ctx).current_team_uid() else {
                    self.show_error_toast(
                        crate::t!("settings-environments-toast-share-no-team"),
                        ctx,
                    );
                    return;
                };
                ...
                let SyncId::ServerId(server_id) = *env_id else {
                    self.show_error_toast(
                        crate::t!("settings-environments-toast-share-not-synced"),
                        ctx,
                    );
                    return;
                };
...
impl EnvironmentsPageWidget {
    fn render_list_page(
        ...
        let title = Text::new(
            crate::t!("settings-environments-page-title"),
        ...
        let description = appearance
            .ui_builder()
            .paragraph(crate::t!("settings-environments-page-description"))
    ...
    fn render_no_matches_state(appearance: &Appearance) -> Box<dyn Element> {
        ...
        Container::new(
            Text::new(
                crate::t!("settings-environments-no-matches"),
                appearance.ui_font_family(),
    ...
    fn render_scoped_section(
        ...
        let header = match list_scope {
            EnvironmentListScope::Personal => Self::render_overline_header(
                &crate::t!("settings-environments-section-personal"),
                appearance,
            ),
            EnvironmentListScope::Team => {
                let shared_by_text = UserWorkspaces::as_ref(app)
                    ...
                    .map(|team| {
                        crate::t!(
                            "settings-environments-section-team-named",
    ...
    fn render_empty_state(
        ...
        let (github_button_label, github_button_enabled) = if dropdown_state.is_loading {
            (crate::t!("settings-environments-button-loading"), false)
        } else if dropdown_state.load_error_message.is_some() {
            (crate::t!("settings-environments-button-retry"), true)
        } else if dropdown_state.auth_url.is_some() {
            (crate::t!("settings-environments-button-authorize"), true)
        } else {
            (crate::t!("settings-environments-button-get-started"), true)
        };
        ...
        let launch_agent_label = crate::t!("settings-environments-button-launch-agent");
        ...
        let github_row = Self::render_empty_state_row(
            ...
            EmptyStateRowConfig {
                icon: Icon::Github,
                title: crate::t!("settings-environments-empty-quick-setup-title"),
                badge: Some(crate::t!("settings-environments-empty-suggested-badge")),
                subtitle: crate::t!("settings-environments-empty-quick-setup-subtitle"),
        ...
        let local_repos_row = Self::render_empty_state_row(
            ...
            EmptyStateRowConfig {
                icon: Icon::Terminal,
                title: crate::t!("settings-environments-empty-use-agent-title"),
                badge: None,
                subtitle: crate::t!("settings-environments-empty-use-agent-subtitle"),
        ...
        let header = Text::new(
            crate::t!("settings-environments-empty-header"),
            appearance.ui_font_family(),
        ...
        let subheader = Text::new(
            crate::t!("settings-environments-empty-subheader"),
            appearance.ui_font_family(),
    ...
    fn render_environment_card(
        ...
        Hoverable::new(card_hover_mouse_state, move |state| {
            ...
            let env_id_with_copy = render_copyable_text_field(
                CopyableTextFieldConfig::new(crate::t!(
            ...
            let mut details_parts = vec![crate::t!(
                "settings-environments-detail-image",
                image = env_docker_image.clone()
            )];
            ...
            if !env_github_repos.is_empty() {
                let repos_text = env_github_repos
                    ...
                    .collect::<Vec<_>>()
                    .join(", ");
                details_parts.push(crate::t!(
                    "settings-environments-detail-repos",
                    repos = repos_text
                ));
            }
            ...
            if !env_setup_commands.is_empty() {
                ...
                details_parts.push(crate::t!(
                    "settings-environments-detail-setup-commands",
            ...
            let view_runs_link = appearance
                .ui_builder()
                .link(
                    crate::t!("settings-environments-view-my-runs"),
                    None,
            ...
            if should_render_share_button {
                ...
                let share_button_element = if is_card_hovered {
                    ...
                    .with_tooltip(move || {
                        share_ui_builder
                            .tool_tip(crate::t!("settings-environments-tooltip-share"))
                            .build()
                            .finish()
                    })
                    .build()
            ...
            if is_card_hovered {
                edit_button = edit_button.with_tooltip(move || {
                    edit_ui_builder
                        .tool_tip(crate::t!("settings-environments-tooltip-edit"))
                        .build()
                        .finish()
                });
            }
...
impl BackingView for EnvironmentsPageView {
    ...
    fn render_header_content(
        ...
        HeaderContent::simple(crate::t!("settings-environments-page-title"))
    }
