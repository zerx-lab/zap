...
pub struct ServerFileBrowserView {
    ...
    upload_batch: Option<ServerFileUploadBatch>,
...
impl ServerFileBrowserView {
    ...
    fn open_context_menu(&mut self, index: usize, position: Vector2F, ctx: &mut ViewContext<Self>) {
        ...
        let menu_items = self.context_menu_items(index, &entry);
    ...
    fn commit_rename(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        ctx.spawn(
            async move {
                rename_remote_path(session, client, remote_session_id, from_path, new_name).await
            },
            |me, result, ctx| {
                me.loading = false;
                match result {
    ...
    fn has_active_upload(&self) -> bool {
        self.upload_batch.as_ref().is_some_and(|batch| {
    ...
    fn active_upload_count(&self) -> usize {
        self.upload_batch.as_ref().map_or(0, |batch| {
            batch
                .tasks
                .iter()
                .filter(|task| {
        ...
        let tasks = pending_files
            ...
            .map(|file| {
                let file_name = file
                    ...
                    .unwrap_or_else(|| file.remote_path.clone());
            ...
            .collect();
        self.upload_batch = Some(ServerFileUploadBatch {
            tasks,
            next_task_index: 0,
    ...
    fn upload_next_task(&mut self, client: Arc<RemoteServerClient>, ctx: &mut ViewContext<Self>) {
        ...
        {
            self.stop_progress_poll();
            self.finish_upload_batch(ctx);
            return;
        }

        let index = self.upload_batch.as_ref().map(|batch| batch.next_task_index).unwrap_or(0);
        if let Some(batch) = self.upload_batch.as_mut() {
        ...
        let (local_path, remote_path, uploaded_bytes) = {
            let batch = self.upload_batch.as_ref().expect("upload batch exists");
        ...
        ctx.spawn(
            async move {
                upload_file_with_progress(client, local_path, remote_path, uploaded_bytes).await
            },
            move |me, result, ctx| {
                if let Some(batch) = me.upload_batch.as_mut() {
    ...
    fn finish_upload_batch(&mut self, ctx: &mut ViewContext<Self>) {
        let first_failure = self.upload_batch.as_ref().and_then(|batch| {
            batch.tasks.iter().find_map(|task| {
        ...
        ctx.open_file_picker(
            move |result, ctx| match result {
                Ok(paths) if !paths.is_empty() => {
                    ...
                    let client_for_collect = client.clone();
                    let client_for_batch = client.clone();
                    let remote_directory = remote_directory.clone();
                    ctx.spawn(
                        async move {
                        ...
                        move |me, result, ctx| match result {
                            Ok(files) if !files.is_empty() => {
                                me.start_upload_batch(client_for_batch, files, ctx);
                            }
                            Ok(_) => {}
    ...
    fn upload_overall_summary(&self) -> Option<(usize, usize)> {
        let batch = self.upload_batch.as_ref()?;
    ...
    fn render_upload_progress_panel(
        ...
        if self
            .upload_batch
            .as_ref()
            .is_some_and(|batch| batch.tasks.iter().any(|t| matches!(t.status, UploadTaskStatus::Completed)))
        {
        ...
        if let Some(batch) = &self.upload_batch {
            if batch.tasks.is_empty() {
                column.add_child(
                    Container::new(
                        Text::new_inline(
...
) -> Result<Vec<PendingUploadFile>, String> {
...
fn upload_task_status_label(task: &ServerFileUploadTask) -> String {
    match &task.status {
