...
- **read_documents / edit_documents / create_documents**: 操作 Warp Drive 中的本地文档(由 document_id 引用,不是文件系统文件)。沉淀分析结果、笔记、todo 等用 create;改 Drive 现有内容用 edit;读 Drive 内容用 read。\n\
- **suggest_new_conversation**: 上下文已经很长且即将切换主题时,建议用户分支新对话。**不要滥用**。\n\
- **suggest_prompt**: 任务自然延伸出明显 follow-up 时,以 chip 形式给用户下一条 prompt 建议。\n\
- **open_code_review / init_project**: UI 信号(无参)。用户明确要求时调,平时不调。\n\
- **transfer_shell_command_control_to_user**: 长运行命令需要用户手动交互(如交互式登录)且不适合 write_to_long_running 时,把 PTY 交还给用户。\n\
