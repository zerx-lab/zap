...
impl PasteAuthTokenModalView {
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        let auth_token_input = ctx.add_typed_action_view(|ctx| {
            ...
            editor.set_placeholder_text(crate::t!("auth-paste-token-placeholder"), ctx);
            editor
        });
...
impl View for PasteAuthTokenModalView {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        let title = FormattedTextElement::from_str(
            crate::t!("auth-paste-token-title"),
        ...
        let subtitle = FormattedTextElement::from_str(
            crate::t!("auth-paste-token-detail"),
            appearance.ui_font_family(),
            14.,
        )
        ...
        let cancel_button = self.cancel_button.render(
            appearance,
            button::Params {
                content: button::Content::Label(crate::t!("auth-paste-token-cancel").into()),
        ...
        let continue_button = self.continue_button.render(
            appearance,
            button::Params {
                content: button::Content::Label(crate::t!("auth-paste-token-continue").into()),
